...
const SummaryCard: React.FC<{ icon: React.ReactElement<any>; title: string; value: string | React.ReactNode; color: string }> = ({ icon, title, value, color }) => (
  <div className="bg-zinc-100/80 p-4 rounded-lg flex items-start gap-4">
    <div className="p-3 mt-1 rounded-full flex items-center justify-center" style={{ backgroundColor: `${color}20`}}>
       {React.cloneElement(icon, { className: `w-6 h-6`, style: { color } })}
    </div>
    <div>
      <p className="text-sm text-zinc-600 font-medium">{title}</p>
      <div className="text-lg font-bold text-zinc-800 break-words">{value}</div>
    </div>
  </div>
);
    ...
    <div className="mb-8">
      ...
      {isDirty && (
        <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-center justify-between gap-4">
            ...
            <button 
                ...
                className="flex-shrink-0 flex items-center bg-amber-500 hover:bg-amber-600 text-white font-semibold py-2 px-4 rounded-lg transition-colors text-sm"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="bg-zinc-100/80 p-4 rounded-lg flex items-start gap-4">
            ...
            <div className="flex-1">
                ...
                <input
                    ...
                    className="text-lg font-bold text-zinc-800 bg-transparent w-full focus:outline-none focus:bg-white/50 focus:ring-1 focus:ring-indigo-400 rounded px-1 -mx-1"
                />
                <textarea
                    ...
                    className="mt-1 text-sm font-normal text-zinc-500 bg-transparent w-full h-auto resize-none focus:outline-none focus:bg-white/50 focus:ring-1 focus:ring-indigo-400 rounded px-1 -mx-1"
                    rows={2}
                />
            </div>
        </div>
        ...
        <SummaryCard 
            ...
            value={<><p>Nr: {invoiceData.invoiceNumber}</p><p className="text-sm font-normal text-zinc-500"> vom {formatDate(invoiceDate)}</p></>}
